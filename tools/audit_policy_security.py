...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Run AXIOM Phase 3 read-only policy/security audit."
    )
    ...
    if args.json:
        ...
        print("AXIOM policy/security audit")
        print("===========================")
        print(f"passed: {payload['passed']}")
        print(f"checked_count: {len(payload['checked'])}")
        ...
        if payload["violations"]:
            for violation in payload["violations"]:
                ...
                if violation["details"]:
                    print(f"  details: {json.dumps(violation['details'], sort_keys=True)}")
        else:
            print("- none")

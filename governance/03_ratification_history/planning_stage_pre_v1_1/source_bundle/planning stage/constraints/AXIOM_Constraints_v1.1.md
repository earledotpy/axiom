**AXIOM Constraints Review — Proposal v1.1**
...
*\*Conditional on RAM accounting below.*

---

### 2. RAM Accounting — Peak Sequential Load

All estimates assume **strictly sequential execution** (no concurrent agent processes). Values derived from legacy ToonTown observations and standard Python/Ollama memory profiles.

| Runtime Component | Estimated RAM Footprint | Notes |
|------------------|------------------------|-------|
| Windows 11 + Python 3.12 base | ~3.1 GB | Observed baseline from legacy |
| Ollama server + qwen3:4b (active, Q4 quantized) | ~1.8–2.2 GB | Must remain memory-mapped; no full-precision load |
| nomic-embed-text (cached) | ~0.3–0.4 GB | Shared process space with Ollama |
| SQLite + sqlite-vec (working set) | ~0.3–0.45 GB | Batch vector ops to limit spikes |
| Active agent process (Goal Planner / Task Planner / Tool Executor) | ~0.2–0.3 GB | Serialized context bundles only |
| Tool Gateways (active subset) | ~0.15–0.2 GB | HTTP buffers, schema validators |
| Sandbox Gateway (when active) | ~0.2–0.3 GB | Isolated process; must enforce memory cap |
| Logging/audit buffers | ~0.1–0.15 GB | JSONL + SQLite write cache |
| **Peak Sequential Total** | **~6.8–7.1 GB** | **Headroom used: ~1.1–1.4 GB of 2.0–2.3 GB available** |

**Conclusion:** The sequential design fits within observed RAM headroom, provided quantization and batching constraints are enforced.

---

### 3. Critical RAM Risk Points — Binding Mitigations
...
### 6. Missing Specification — Minor Blocker
...
**AXIOM Proposal v1.1 is FEASIBLE** on the target hardware (Intel Celeron N4500, 8 GB RAM, Windows 11, no GPU) **subject to the following binding conditions**:
...
**Next Step:** Proposal v1.1 may advance to DeepSeek V4 — Adversarial Critic for security and failure-mode review, pending attachment of this feasibility ruling.
